AFRAME.registerPrimitive('a-forest', {
  defaultComponents: {
    forest: {}
  },
  mappings: {
    distance: 'forest.distance'
  }
});
...
AFRAME.registerComponent('forest', {
  ...
  init: function() {
    ...
    map.forEach(function(line, i) {
      line.forEach(function(tile, j) {
        if (tile === 'tree') {
          self.trees.push({
            x: j * 5 + Math2.rangeRandom(-2, 2),
            y: 0,
            z: i * 5 + Math2.rangeRandom(-2, 2),
            id: treeCount
          });
          treeCount++;
        }
  ...
  tick: function() {
    ...
    this.trees.forEach(function(tree) {
      if (Math2.distance(tree, userPosition) <= self.data.distance) {
        neededPositions.push(tree);
      }
    });
    var currentTrees = Array.prototype.slice.call(this.el.childNodes, 0);
    neededPositions = neededPositions.filter(function(p) {
      return !currentTrees.find(c => c.getAttribute('tree-id') == p.id);
    });
    currentTrees.filter(function(c) {
      return Math2.distance(c.getAttribute('position'), userPosition) > self.data.distance;
    }).forEach(function(tree) {
      self.el.removeChild(tree);
    });

    neededPositions.forEach(function(p) {
      var newTree = document.createElement('a-tree');
      newTree.setAttribute('position', p.x + ' ' + p.y + ' ' + p.z);
      newTree.setAttribute('tree-id', p.id);
      self.el.appendChild(newTree);
    });
  }
})
